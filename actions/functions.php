...
function getSearchResult($search) {
    ...
    $query = "SELECT * FROM product where product_name like '%".$search."%' or product_desc like '%".$search."%' or more_details like '%".$search."%'";
        $result = mysql_query($query);
        if (!$result) die ("Database access failed: " . mysql_error());
        $rows = mysql_num_rows($result);
        if($rows==0){
...
function getCategoryName($category_id){
    $query = "SELECT category_name FROM category where id='".$category_id."'";
        ...
        $rows = mysql_num_rows($result);
        if($rows>0){
           return mysql_result($result,0,'category_name');
        }
        else {
            echo 'No Category';
        }
